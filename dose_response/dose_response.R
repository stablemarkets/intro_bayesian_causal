...
for(i in 1:N){
  A[i] = sample(0:(K-1), 1, prob = invlogit( seq(1,-1,length.out = K) + L[i] ) )
}
...
Y = rnorm(n = N, mean =  5*pnorm( A - 5) - 5*L, sd = 3 )
...
####-------------------         Plot Results            --------------------####
dose = 1:(K-1)
true_Psi = 5*pnorm(dose-5) - 5*pnorm(0:(K-2)-5)
...
lines(dose, colMeans(Psi_draws), col='blue', type='o', pch=20)
...
legend('bottomleft', 
       ...
       col = c('blue', 'black', 'red'), pch=c(20,20,20), lty=c(1,1,1),
       bty='n')
dev.off()